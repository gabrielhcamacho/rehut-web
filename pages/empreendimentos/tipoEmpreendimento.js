...
export default function tipoEmpreendimento() {
    ...
    const handleNextClick = (selectedTipo) => {
        if (!selectedTipo) {
            setShowAlert(true);
            return;
        }

        const tipo = selectedTipo.toLowerCase()
        // Direcionar para a próxima tela com o tipo selecionado na URL
        router.push(`criar/${tipo}`);
    };
                ...
                <Board>
                    ...
                    <div className='bg-white h-max shadow-md rounded-md mt-2'>
                        <div className="flex flex-col p-5 w-full ">
                            <span className='text-2xl text-text font-semibold ml-3 mt-2'>Escolha o Que Deseja Cadastrar</span>
                            ...
                            <div className='grid grid-cols-4 px-2 auto-rows-auto gap-y-2 gap-x-10 items-center justify-center mt-4 h-[660px]'>
                                {tipoEmpreendimentos.map((item) => (
                                    <div
                                        key={item.title}
                                        className="flex group flex-col items-center justify-between max-h-[275px] rounded-lg h-full border border-slate-200 p-8 cursor-pointer hover:bg-slate-200 hover:border-slate-300"
                                        onClick={() => handleNextClick(item.title)}
                                    >
                                        <div className="flex-grow flex items-center justify-center max-h-[150px]">
                                            <Image src={item.img} className="max-h-full max-w-full" />
                                        </div>
                                        <p className="text-text font-normal mt-9 group-hover:font-semibold">
                                            {item.title}
                                        </p>
                                    </div>
                                ))}
